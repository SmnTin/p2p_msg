...
namespace p2p {
    namespace DHT {
        ...
        RoutingTable::RoutingTable(DHT * dht) : _dht(dht) {
            _id = NodeId::random();
        ...
        RoutingTable::~RoutingTable() {
            if(_pingNodesTask)
                _pingNodesTask->cancel();
        }

        const NodeId RoutingTable::getId() {
            std::lock_guard<std::recursive_mutex> guard(_idMutex);

            return _id;
        }

        KBuckets::iterator RoutingTable::findBucket(const p2p::NodeId &nodeId) {
            std::lock_guard<std::recursive_mutex> guard(_bucketsMutex);
            std::lock_guard<std::recursive_mutex> guard2(_idMutex);
        ...
        bool RoutingTable::putNode(std::shared_ptr<p2p::Node> node) {
            std::lock_guard<std::recursive_mutex> guard(_bucketsMutex);
...
//                }
                ...
                (*containsIt)->address() = node->address();
        ...
        std::vector<std::shared_ptr<Node>> RoutingTable::findNearestNodes(const p2p::NodeId &nodeId,
                                                                          std::size_t count) {
            std::lock_guard<std::recursive_mutex> guard(_bucketsMutex);
...
//                ans.push_back(std::make_shared<Node>(getId(), _networkEngine->getThisEndpoint()));
        ...
        void RoutingTable::pingNode(const boost::asio::ip::udp::endpoint &endp) {
            _dht->getMessageHandler()->pingNode(endp);
        }

        void RoutingTable::pingNodesTaskFunc() {
            std::lock_guard<std::recursive_mutex> guard(_bucketsMutex);
            for(auto & bucket : _buckets)
                for(auto & node : bucket)
                    pingNode(node->address());

            _pingNodesTask = _dht->getSheduler()->executeTaskIn(std::chrono::milliseconds {PING_DELAY * 1000},
                    std::bind(&RoutingTable::pingNodesTaskFunc, this));
        }

        std::shared_ptr<Node> RoutingTable::findNode(const NodeId &nodeId) {
            auto it = findBucket(nodeId);
        ...
        std::ostream & operator<< (std::ostream & out, RoutingTable & table) {
            std::lock_guard<std::recursive_mutex> guard(table._bucketsMutex);
            std::lock_guard<std::recursive_mutex> guard2(table._idMutex);
